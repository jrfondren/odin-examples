...
main :: proc() {
	...
	for !rl.WindowShouldClose() { 	// Detect window close button or ESC key
		...
		if playerCellX <
		   0 {playerCellX = 0} else if playerCellX >= cubicmap.width {playerCellX = cubicmap.width - 1}

		if playerCellY <
		   0 {playerCellY = 0} else if playerCellY >= cubicmap.height {playerCellY = cubicmap.height - 1}
			...
			for x in 0 ..< cubicmap.width {
				...
				if mapPixels[y * cubicmap.width + x].r == 255 &&
				   rl.CheckCollisionCircleRec(
					   playerPos,
					   playerRadius,
					   {mapPosition.x - 0.5 + f32(x), mapPosition.z - 0.5 + f32(y), 1, 1},
				   ) {
		...
		rl.DrawTextureEx(
			cubicmap,
			{f32(rl.GetScreenWidth() - cubicmap.width * 4 - 20), 20},
			0,
			4,
			rl.WHITE,
		)
